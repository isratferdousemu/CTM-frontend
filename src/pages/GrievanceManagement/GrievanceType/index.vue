<template>
  <div id="division">
    <v-row class="mx-5 mt-4">
            ...
            <v-card elevation="10" color="white" rounded="md" theme="light" class="mb-8">
              ...
              <v-card-text>
                ...
                <v-row class="ma-0 pa-3 white round-border d-flex justify-space-between align-center" justify="center"
                  ...
                  <v-col cols="12">
                    <v-data-table :loading="loading" item-key="id" :headers="headers" :items="grievanceType"
      ...
      <!-- Grievance Type add modal  -->
      <v-dialog v-model="dialogAdd" width="650">
        <v-card style="justify-content: center; text-align: center">
          <v-card-title class="font-weight-bold justify-center">
            {{ $t("container.grievance_management.add_grievance_type") }}
          </v-card-title>
          <v-divider></v-divider>
          <v-card-text class="mt-7">
            <ValidationObserver ref="formAdd" v-slot="{ invalid }">
              <form @submit.prevent="submitGrievanceType()">
                <!-- {{errors.code}}
                  {{errors.name_en}} -->
      ...
      <!-- Grievance Type Edit modal  -->
      <v-dialog v-model="dialogEdit" width="650">
        <v-card style="justify-content: center; text-align: center">
          <v-card-title class="font-weight-bold justify-center">
            {{ $t("container.grievance_management.edit_grievance_type") }}
          </v-card-title>
          <v-divider></v-divider>
          <v-card-text class="mt-7">
            <ValidationObserver ref="formEdit" v-slot="{ invalid }">
              <form @submit.prevent="updateGrievanceType()">
                ...
                <ValidationProvider name="Title English" vid="title_en" v-slot="{ errors }" rules="required">
                  <v-text-field outlined type="text" v-model="data.title_en" :label="$t('container.grievance_management.title_en')
                    " required :error="errors[0] ? true : false" :error-messages="errors[0]"></v-text-field>
                </ValidationProvider>
      ...
      <v-dialog v-model="deleteDialog" width="350">
        <v-card style="justify-content: center; text-align: center">
          <v-card-title class="font-weight-bold justify-center">
            {{ $t("container.grievance_management.grievance_delete") }}
          </v-card-title>
          <v-divider></v-divider>
          <v-card-text>
            <div class="subtitle-1 font-weight-medium mt-5">
              {{
                $t("container.grievance_management.grievance_delete_message")
              }}
            </div>
          </v-card-text>
          <v-card-actions style="display: block">
            <v-row class="mx-0 my-0 py-2" justify="center">
              ...
              <v-btn text @click="deleteGetGrievanceType()" color="white" :loading="delete_loading"
                class="custom-btn-width warning white--text py-2">
  ...
  methods: {
    ...
    async GeneratePDF() {
      ...
      await this.$axios
        ...
        .then((result) => {
          this.AllGrievanceType = result.data.data;
        });
      ...
      const queryParam = {
        ...
        data: CustomInfo,
        header: HeaderInfo,
        fileName: this.$t("container.grievance_management.grievanceType"),
      };
      try {
    ...
    setInitialHeader() {
      for (let i = 0; i < this.headers.length; i++) {
        if (this.headers[i].value == "title_en") {
    ...
    deleteGetGrievanceType: async function () {
      try {
        this.$axios
          .delete(`/admin/grievanceType/destroy/${this.delete_id}`, {
            headers: {
              Authorization: "Bearer " + this.$store.state.token,
            },
