<template>
    <div id="success">
        <v-app-bar color="#405c61" fixed height="80" class="px-4" dense dark>
            <v-img class="p-3 mr-4" max-height="100%" max-width="60px" position="center center"
                src="/assets/images/logo.png"></v-img>
            <v-toolbar-title>সামাজিক নিরাপত্তা কর্মসূচী সমাজসেবা অধিদফতর</v-toolbar-title>
        </v-app-bar>
        <v-row class="mt-10">
            <v-col cols="6" class="mx-auto mt-10">
                <v-card>
                    <v-card-text class="mt-10 text-center">
                        <p style="font-size: 40px" class="green--text mt-5">Application Submitted Successfully</p>
                        <p style="font-size: 20px" class="mt-5">Your Tracking Id is {{ successId }}</p>
                    </v-card-text>
                </v-card>
            </v-col>
        </v-row>
    </div>
</template>

<script>
export default {
    computed: {
        successId() {
            console.log('this.$store:', this.$store);
            console.log('this.$store.state.ApplicationSelection:', this.$store.state.ApplicationSelection);
            return this.$store.state.ApplicationSelection.successId;
        },
    },
};
</script>
