                  ...
                  <v-row align="center" justify="center">
                    <v-col>
                      <p style="font-size: 20px">
                        {{
                          $t(
                              "container.application_selection.application.new_applicant"
                          )
                        }}
                      </p>
...
export default {
  ...
  data() {
    return {
      ...
      asset_url: process.env.VUE_APP_BASE_URL_BACKEND,
      genders: [],
      imageUrl: null,
  ...
  methods: {
    ...
    update() {
      ...
      try {
        ...
        // );
        // formData.append("nominee_date_of_birth", this.data.nominee_date_of_birth);
        // if (this.data.nominee_image instanceof File) {
        //   formData.append("nominee_image", this.data.nominee_image);
        // }
        // if (this.data.nominee_signature instanceof File) {
        ...
        // formData.append("branch_name", this.data.branch_name);
        ApiService
            .update(`/admin/emergency/beneficiary/update/${this.editItem.id}`, this.data)
            .then((res) => {
              console.log(res);
              if (res?.data?.success) {
                this.$toast.success(
    ...
    previewImageNominee() {
      if (this.data.nominee_image) {
        ...
        reader.onload = (e) => {
          ...
          console.log(this.nomineeImageUrl)
        };
    ...
    loadEditableForm() {
      ...
      if (edit_id) {
        ApiService.get(`admin/emergency/beneficiary/edit/${edit_id}`)
            .then((res) => {
              console.log(res);
    ...
    setEditData(item) {
      ...
      this.data.emergency_allotment_id = item.allotment_id;
      this.data.mobile = item.current_mobile;
      this.data.division_id = item.current_division_id;
      this.data.district_id = item.current_district_id;
      this.getDistrictList(item.current_division_id);
      this.data.location_type = item.current_location_type;
      this.LocationType(item.current_location_type);
      this.data.post_code = item.current_post_code;
      this.data.address = item.current_address;
      this.imageUrl = this.asset_url + '/storage/' + this.data.image;
      this.signUrl = this.asset_url + '/storage/' + item?.signature;
      this.nomineeImageUrl = this.asset_url + '/storage/' + item?.nominee_image;
      this.nomineeSignUrl = this.asset_url + '/storage/' + item?.nominee_signature;
      if (item.location_type === item.permanent_location_type) {
        this.checkbox = true;
      }
      /*present location*/
      // District Paurashova
      if (this.data.location_type === 1) {
        this.data.district_pouro_id = item.current_district_pourashava_id;
        this.data.ward_id_dist = item.current_ward_id;
        this.onChangeDistrictPouro(this.data.district_pouro_id);

      }
      // Upazila union pauroshova ward

      if (this.data.location_type === 2) {
        this.data.thana_id = item.current_upazila_id;
        if (this.data.sub_location_type === 2) {
          this.SubLocationType(2);
          this.data.union_id = item.current_union_id;
          this.getUnionList(item.current_upazila_id);
          this.data.ward_id_union = item.current_ward_id;
          this.onChangeUnion(this.data.union_id);
        } else {
          this.SubLocationType(1);
          this.data.pouro_id = item.current_pourashava_id;
          this.data.ward_id_pouro =
              this.onChangePouro(item.current_pourashava_id);
        }

      }
      //City Corporation thana ward
      if (this.data.location_type === 3) {
        this.data.city_id = item.current_city_corp_id;
        this.data.city_thana_id = item.current_thana_id;
        this.getCityThanaList(this.data.city_id);
        this.data.ward_id_city = item.current_ward_id;
        this.OnChangeCityThana(this.data.city_thana_id);
      }

      /*Permanent location*/

      this.data.permanent_division_id = item.permanent_division_id;
      this.data.permanent_district_id = item.permanent_district_id;
      this.getPermanentDistrictList(this.data.permanent_division_id);
      this.data.permanent_location_type = item.permanent_location_type;
      this.permanent_locationType(item.permanent_location_type);
      this.data.permanent_post_code = item.permanent_post_code;
      this.data.permanent_address = item.permanent_address;


      // Permanent District Paurashova
      if (this.data.permanent_location_type === 1) {
        this.data.permanent_district_pouro_id = item.permanent_district_pourashava_id;
        this.data.permanent_ward_id_dist = item.permanent_ward_id;
      ...
      if (this.data.permanent_location_type === 2) {
        this.data.permanent_thana_id = item.permanent_upazila_id;
        if (this.data.permanent_sub_location_type === 2) {
          this.permanentSubLocationType(2);
          this.data.permanent_union_id = item.permanent_union_id;
          this.getUnionList(this.data.permanent_thana_id);
          this.data.permanent_ward_id_union = item.permanent_ward_id;
          this.permanent_onChangeUnion(item.permanent_union_id);
        } else {
          this.permanentSubLocationType(1);
          this.data.permanent_pouro_id = item.permanent_pourashava_id;
          this.data.permanent_ward_id_pouro =
              this.Permanent_onChangePouro(item.permanent_pourashava_id);
        }

      }
      //Permanent City Corporation thana ward
      if (this.data.permanent_location_type === 3) {
        this.data.permanent_city_id = item.permanent_city_corp_id;
        this.data.permanent_city_thana_id = item.current_thana_id;
        this.getPermanentCityThanaList(this.data.permanent_city_id);
        this.data.permanent_ward_id_city = item.permanent_ward_id;
        this.Permanent_OnChangeCityThana(this.data.permanent_city_thana_id);
      }
