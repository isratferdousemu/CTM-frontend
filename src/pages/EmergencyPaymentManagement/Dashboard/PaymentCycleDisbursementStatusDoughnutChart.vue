<template>
  <v-col>
        ...
        >
          <v-card-title style="padding: 10px">
            <h5 class="white--text">
              {{
                $t(
                  "container.payroll_management.emergency_dashboard.payment_cycle_disbursment_status"
                )
              }}
            </h5>
          </v-card-title>
    ...
    <v-row>
      <img
        v-if="allZeros == true"
        style="margin-left: 80px; margin-top: 10px; width: 300px; height: 300px"
        src="/assets/images/pie_chart_default.png"
        alt="default chart"
      />
      <canvas v-else id="payment_cycle_disbursement_status"></canvas>
    </v-row>
  </v-col>
</template>
...
export default {
  data() {
    return {
      ...
      payment_cycle_disbursement_status: [],
      levels: [],
      datas: [],
      programs: [],
      program_id: null,
  ...
  methods: {
    ...
    async getData(status, from_date = null, to_date = null) {
      ...
      try {
        const result = await this.$axios.get(
          "admin/payroll/payment-cycle-disbursement-status",
          {
            headers: {
              Authorization: "Bearer " + this.$store.state.token,
              "Content-Type": "multipart/form-data",
    ...
    createChart() {
      ...
      if (this.levels && this.datas) {
        ...
        this.total_number_of_application_received_chart = new Chart(
          document.getElementById("payment_cycle_disbursement_status"),
          {
            type: "doughnut",
            data: {
              // labels: this.levels,
              // labels: this.levels.map((label, index) => `${label} (${percentages[index]})`),
              labels: this.levels.map(
                (label, index) =>
                  `${label} (${
                    this.$i18n.locale == "en"
                      ? this.datas[index]
                      : this.$helpers.englishToBangla(this.datas[index])
                  } - ${
                    this.$i18n.locale == "en"
                      ? percentages[index]
              ...
              datasets: [
                {
                  label: "Count",
                  backgroundColor: this.datas.map(() =>
        ...
        document.getElementById("payment_cycle_disbursement_status").style.width = "400px";
        document.getElementById("payment_cycle_disbursement_status").style.height = "435px";
      } else {
