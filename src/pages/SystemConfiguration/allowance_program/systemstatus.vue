                            ...
                            <v-card-text>
                                <v-card-title class="mb-5">
                                    <div class="d-flex justify-sm-end flex-wrap">
                                        <v-text-field @keyup.native="getAllowance" v-model="search"
                                ...
                                <v-card-subtitle>
                                    <v-data-table :headers="headers" :items="allowances" :search="search"
                                        :options.sync="options" :server-items-length="totalAllowances" :loading="loading"
                                        :footer-props="{
                                        ...
                                        <template v-slot:[`item.actions`]="{ item }" style="padding: 10px;">
                                            <v-tooltip top>
                                                <template v-slot:activator="{ on }">
                                                    <v-btn :disabled="item.default === 1" fab style="margin-right: 10px;"
                                                        x-small color="success" v-on="on" router
                                            ...
                                            <v-tooltip top>
                                                <template v-slot:activator="{ on }">
                                                    <v-btn :disabled="item.default === 1" fab x-small color="success"
                                                        v-on="on" router
                                            ...
                                            <v-tooltip top>
                                                <template v-slot:activator="{ on }">
                                                    <v-btn :disabled="item.default === 1" fab x-small color="grey"
                                                        class="ml-3 white--text" v-on="on" @click="deleteAlert(item.id)">
                ...
                <v-card style="justify-content: center; text-align: center">
                    ...
                    <v-card-text>
                        <div class="subtitle-1 font-weight-medium mt-5">
                            Are you sure to delete this allowance program? Allowance program all information will be
                            deleted.
                        </div>
                    </v-card-text>
