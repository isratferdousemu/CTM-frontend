          ...
          <v-col cols="12">
            <v-card elevation="10" color="white" rounded="md" theme="light" class="mb-8">
              <v-card-title class="justify-center" tag="div">
                <h3 class="text-uppercase pt-3"> {{ $t("container.system_config.demo_graphic.district.list") }} </h3>
              </v-card-title>
              <v-card-text>
                <v-row class="ma-0 pa-3 white round-border d-flex justify-space-between align-center" justify="center"
                  justify-lg="space-between">
                  <div class="d-flex justify-sm-end flex-wrap">
                    <v-text-field @keyup.native="GetDistrict" outlined dense v-model="search"
                      prepend-inner-icon="mdi-magnify" class="my-sm-0 my-3 mx-0v -input--horizontal" flat
                      variant="outlined" :label='$t("container.system_config.demo_graphic.district.search")' hide-details
                      color="primary">
                    </v-text-field>
                  </div>
                  ...
                  <v-col cols="12">
                    <v-data-table :loading="loading" item-key="id" :headers="headers" :items="districts"
                      ...
                      <template v-slot:item.actions="{ item }">
                        ...
                        <v-btn v-can="'delete-district'" fab x-small color="grey" class="ml-3 white--text" elevation="0"
              ...
              <form @submit.prevent="submitDistrict()">
                ...
                <ValidationProvider name="Division" vid="division" rules="required">
                  <v-autocomplete @input="onChangeDivision($event)" v-model="data.division_id" outlined
                    :label='$t("container.system_config.demo_graphic.division.division")' :items="divisions"
          ...
          <v-card-actions style="display: block">
            <v-row class="mx-0 my-0 py-2" justify="center">
             ...
             {{ $t("container.list.cancel") }}
              ...
              <v-btn text @click="deleteDistrict()" color="white" :loading="delete_loading"
                class="custom-btn-width warning white--text py-2">
  ...
  computed: {
    headers() {
      return [
        ...
        { text: this.$t("container.system_config.demo_graphic.division.division"), value: "division.name_en" },
