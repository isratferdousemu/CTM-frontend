                ...
                <v-row class="ma-0 pa-3 white round-border d-flex justify-space-between align-center " justify="center"
                  ...
                  <div class="d-flex justify-sm-end flex-wrap">
                    <v-text-field @keyup.native="GetWard" outlined dense v-model="search" prepend-inner-icon="mdi-magnify"
                      class="my-sm-0 my-3 mx-0v -input--horizontal " flat variant="outlined" label="search thana/upazila"
                  ...
                  <v-col cols="12">
                    <v-data-table :loading="loading" item-key="id" :headers="headers" :items="wards"
                      ...
                      <template v-slot:item.division="{ item }">
                        {{ item.union.thana.district.division.name_en }}
                      </template>
                      <template v-slot:item.district="{ item }">
                        {{ item.union.thana.district.name_en }}
                      </template>
                      <template v-slot:item.thana="{ item }">
                        {{ item.union.thana.name_en }}
                      </template>
                ...
                <v-row>
                  ...
                  <v-col>
                    ...
                    <ValidationProvider name="Union/pourashava" vid="division" rules="required" v-slot="{ errors }">
                      <v-autocomplete v-if="data.location_type == '1'" v-model="data.union_id" outlined
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: "Ward  (EN)", value: "name_en" },
        { text: "Ward  (BN)", value: "name_bn" },
  ...
  methods: {
    ...
    async updateWard() {
      try {
        this.$store.dispatch("Ward/UpdateWard", this.data).then(() => {
          if (this.error_status == "") {
    ...
    async onChangeThana(event) {
      ...
      await axios.get(`/admin/union/get/${event}`, {
        headers: {
          Authorization: "Bearer " + this.$store.state.token,
          "Content-Type": "multipart/form-data",
        }
    ...
    editWard(item) {
      ...
      this.data.division_id = item.union.thana.district.division.id;

      this.data.district_id = item.union.thana.district.id;
      this.data.thana_id = item.union.thana.id;
      this.data.union_id = item.union.id;
