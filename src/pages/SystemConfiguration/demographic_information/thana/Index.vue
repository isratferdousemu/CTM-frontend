              ...
              <v-card-text>
                <v-row class="ma-0 pa-3 white round-border d-flex justify-space-between align-center " justify="center"
                  ...
                  <div class="d-flex justify-sm-end flex-wrap">
                    <v-text-field @keyup.native="GetThana" outlined dense v-model="search"
                  ...
                  <v-btn @click="dialogAdd = true" flat color="primary" prepend-icon="mdi-account-multiple-plus">
            ...
            <ValidationObserver ref="form" v-slot="{ invalid }">
              <form @submit.prevent="submitUpazila()">
                ...
                <ValidationProvider name="Division" vid="division" rules="required" v-slot="{ errors }">
                  <v-autocomplete @input="onChangeDivision($event)" v-model="data.division_id" outlined label="Division"
          ...
          <v-card-text class="mt-7">
            <ValidationObserver ref="form" v-slot="{ invalid }">
              <form @submit.prevent="updateUpazila()">
                <ValidationProvider name="Code" vid="code" rules="required" v-slot="{ errors }">
                  <v-text-field outlined type="text" v-model="editdata.code" label="Code" required
                    :error="errors[0] ? true : false" :error-messages="errors[0]"></v-text-field>
                </ValidationProvider>
                <ValidationProvider name="Division" vid="division" rules="required" v-slot="{ errors }">
                  <v-autocomplete @input="onChangeDivision($event)" v-model="editdata.division_id" outlined label="Division"
                    :items="divisions" item-text="name_en" item-value="id" required :error="errors[0] ? true : false"
                    :error-messages="errors[0]"></v-autocomplete>
                </ValidationProvider>
                <ValidationProvider name="District" vid="district" rules="required" v-slot="{ errors }">
                  <v-autocomplete outlined v-model="editdata.district_id" label="District" :items="districts"
                    item-text="name_en" item-value="id" required :error="errors[0] ? true : false"
                    :error-messages="errors[0]"></v-autocomplete>
                </ValidationProvider>
                <ValidationProvider name="Location_type" vid="location_type" rules="required" v-slot="{ errors }">
                  <v-autocomplete outlined v-model="editdata.location_type" label="Location Type" :items="locationTypes"
                    item-text="value_en" item-value="id" required :error="errors[0] ? true : false"
                    :error-messages="errors[0]"></v-autocomplete>
                </ValidationProvider>
                <ValidationProvider name="Name English" vid="name_en" rules="required" v-slot="{ errors }">
                  <v-text-field outlined type="text" v-model="editdata.name_en" label="Name English" required
                    :error="errors[0] ? true : false" :error-messages="errors[0]"></v-text-field>
                </ValidationProvider>
                <ValidationProvider name="Name Bangla" vid="name_bn" rules="required" v-slot="{ errors }">
                  <v-text-field outlined type="text" v-model="editdata.name_bn" label="Name Bangla" required
                    :error="errors[0] ? true : false" :error-messages="errors[0]"></v-text-field>
                </ValidationProvider>
...
export default {
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        { text: "Thana/Upazila Name English", value: "name_en" },
        { text: "Thana/Upazila Name English Bangla", value: "name_bn" },
    ...
    ...mapState({
      ...
      errors: (state) => state.Thana.errors,
  ...
  methods: {
    async submitUpazila() {
      ...
      //   await this.$store.dispatch("Thana/StoreUpazila", this.data);
    ...
    editUpazila(item) {
      ...
      this.editdata.name_en = item.name_en;
      this.editdata.name_bn = item.name_bn;
      this.editdata.code = item.code
      this.editdata.division_id = item.district.division.id;
      
      // this.data.district_id = item.district.id;
      this.data.location_type = item.location_type.id;
      console.log(this.data.division_id)

       axios.get(`/admin/district/get/${this.data.division_id}`, {
        headers: {
          Authorization: "Bearer " + this.$store.state.token,
          "Content-Type": "multipart/form-data",
        }
        ...
        this.districts = result.data.data;
        this.data.district_id=this.districts.id
        console.log(this.districts)
