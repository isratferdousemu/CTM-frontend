        ...
        <v-row>
          <v-col cols="12">
            <div class="d-block text-right">
              <v-btn v-can="'beneficiaryShifting-create'" elevation="2" class="btn" color="primary" router
              ...
              <v-col cols="12">
                ...
                ">
                    <h3 class="white--text text-uppercase pt-3">
                      {{ $t("container.beneficiary_management.beneficiary_shifting.location_shitfing_list") }}
                    </h3>
                  </v-card-title>
                  ...
                  <v-card-text>
                    <v-row class="ma-0 white round-border d-flex justify-space-between align-center" justify="center"
                      ...
                      <v-col cols="12">
                        <v-data-table :headers="headers" :items="beneficiaries" :loading="loading" item-key="id"
                            ...
                            <v-tooltip top>
                              <template v-slot:activator="{ on }">
                                <v-btn v-can="'update-post'" fab x-small v-on="on" color="#AFB42B" elevation="0"
                                  ...
                                  :to="`/beneficiary-management/beneficiary-info/details/${item.beneficiary_id}`" target="_blank">
...
export default {
  name: "Index",
  title: "CTM - Beneficiary Location Shifting List",
  data() {
    return {
      data: {
        program_id: null,
  ...
  computed: {
    ...
    headers() {
      return [
        ...
        {
          text: this.$t(
            "container.beneficiary_management.beneficiary_list.shifting_cause"
          ),
          value: "shifting_cause",
        },
        {
          text: this.$t(
  ...
  methods: {
    ...
    async GeneratePDF() {
      ...
      this.$axios
        .get("/admin/beneficiary/getBeneficiaryShiftingListPdf", {
          headers: {
            Authorization: "Bearer " + this.$store.state.token,
    ...
    async GenerateExcel() {
      ...
      await this.$axios
        ...
        .then((result) => {
          this.beneficiary_shifing_list = result.data.data;
          this.isLoading = false;
        })
        .catch((error) => {
          this.isLoading = false;
      ...
      try {
        import("@/plugins/Export2Excel").then((excel) => {
          ...
          const CustomInfo = this.beneficiary_shifing_list.map((i, index) => {
            return {
              sl:
                this.$i18n.locale == "en"
                  ? index + 1
              ...
              city_dist_upazilla:
                this.$i18n.locale == "en"
                  ? i.upazila_name_en
                    ? i.upazila_name_en
                    : i.city_corporation_name_en
                      ? i.city_corporation_name_en
                      : i.district_pourashava_name_en
                        ? i.district_pourashava_name_en
                        : ""
                  : i.upazila_name_bn
                    ? i.upazila_name_bn
                    : i.city_corporation_name_bn
                      ? i.city_corporation_name_bn
                      : i.district_pourashava_name_bn
                        ? i.district_pourashava_name_bn
                        : "",
              thna_union_Pouro:
                this.$i18n.locale == "en"
                  ...
                  : i.union_bn
                    ...
                    : i.thana_bn
                      ? i.thana_bn
                      : i.pourashava_name_bn
                        ? i.pourashava_name_bn
                        : "",
              ward: this.$i18n.locale == "en" ? i.ward_en : i.ward_bn,
              prev_program_name:
                this.$i18n.locale == "en"
              ...
              activation_date:
                this.$i18n.locale == "en"
                  ? i.activation_date
                  : this.$helpers.englishToBangla(i.activation_date),
            };
          });
