        ...
        <v-row>
          <v-col cols="12">
            <div class="d-block text-right">
              <v-btn v-can="'beneficiaryExit-view'" elevation="2" class="btn my-2" color="primary" router
                to="/beneficiary-management/beneficiary-exit-list">
            ...
            <v-expansion-panels>
              <v-expansion-panel>
                ...
                " :items="programs" item-text="name_en" item-value="id" class="no-arrow-icon"
                        ...
                        <v-col lg="3" md="3" cols="12">
                          <ValidationProvider name="Division" vid="division" v-slot="{ errors }">
                            <v-autocomplete outlined @input="onChangeDivision($event)" v-model="data.division_id"
                              :label="$t(
                ...
                " @change="onChangeUpazila($event)" :items="thanas" item-text="name_en" item-value="id" class="no-arrow-icon"
                              :append-icon-cb="appendIconCallback" append-icon="mdi-plus"
                              :error="errors[0] ? true : false" :error-messages="errors[0]"></v-autocomplete>
                          </ValidationProvider>
                        </v-col>
                ...
                " @change="onChangeUnion($event)" :items="unions" item-text="name_en" item-value="id" class="no-arrow-icon"
                              :append-icon-cb="appendIconCallback" append-icon="mdi-plus"
                              :error="errors[0] ? true : false" :error-messages="errors[0]"></v-autocomplete>
                          </ValidationProvider>
                        </v-col>
                        ...
                        <v-col v-if="data.location_type == 3" lg="3" md="3" cols="12">
                          <ValidationProvider name="city" vid="city_id" v-slot="{ errors }">
                            <v-autocomplete v-model="data.city_id" @change="onChangeCity($event)" outlined :label="$t('container.system_config.demo_graphic.ward.city')
                ...
                " lg="3" md="3" cols="12">
                          <ValidationProvider name="ward" vid="ward_id" v-slot="{ errors }">
                            <v-autocomplete v-model="data.ward_id" outlined :label="$t('container.system_config.demo_graphic.ward.ward')
            ...
            <v-row>
              <v-col cols="12">
                <v-card elevation="10" color="white" rounded="md" theme="light" class="mt-2 mb-3">
                    ...
                    <form @submit.prevent="onBeneficiaryExit()">
                      <v-row class="ml-2 mr-2">
                        <v-col lg="6" md="6" cols="12">
                          <ValidationProvider name="Reason" vid="data.exit_reason_id" rules="required"
                            v-slot="{ errors }">
                            <v-autocomplete :hide-details="errors[0] ? false : true" v-model="data.exit_reason_id"
                              outlined :label="$t('container.list.reason')" :items="cause_types" item-text="value_en"
                              item-value="id" required clearable :error="errors[0] ? true : false"
                              :error-messages="errors[0]"></v-autocomplete>
                          </ValidationProvider>
                        </v-col>

                        <v-col lg="6" md="6" cols="12">
                          <v-text-field :label="$t('container.list.reason_details')" outlined
                            v-model="data.exit_reason_detail">
                          </v-text-field>
                        </v-col>
                        <v-col lg="6" md="6" cols="12">
                ...
                $t("container.list.reset")
                        ...
                        <v-btn elevation="2" class="btn mr-2 mb-2" color="success" type="submit" flat
                          :disabled="invalid">{{
  ...
  methods: {
    ...
    async onBeneficiaryExit() {
      ...
      if (beneficiaries.length === 0) {
        this.$toast.error("Please select at least one beneficiary to exit");
        return;
      }
