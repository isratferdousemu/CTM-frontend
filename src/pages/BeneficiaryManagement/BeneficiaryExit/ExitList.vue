  ...
  methods: {
    ...
    async GenerateExcel() {
      ...
      await this.$axios
        ...
        .then((result) => {
          this.beneficiary_exit_list = result.data.data;
          this.isLoading = false;
        })
        .catch((error) => {
          this.isLoading = false;
      ...
      try {
        import("@/plugins/Export2Excel").then((excel) => {
          const HeaderInfo = [
            ...
            this.$t(
              "container.beneficiary_management.beneficiary_list.exit_reason"
            ),
            this.$t(
              "container.beneficiary_management.beneficiary_list.exit_date"
            ),
          ];
          const CustomInfo = this.beneficiary_exit_list.map((i, index) => {
            return {
              sl:
                this.$i18n.locale == "en"
                  ? index + 1
              ...
              exit_reason:
                this.$i18n.locale == "en" ? i.exit_reason_en : i.exit_reason_bn,
              exit_date:
                this.$i18n.locale == "en"
                  ? i.exit_date
                  : this.$helpers.englishToBangla(i.exit_date),
            };
          });
