<template lang="">
<v-app-bar elevation="0" height="70" :clipped-left="clipped" fixed app class="mx-5 my-3 rounded-lg header-left menu-top" style="box-shadow: 3px 8px 14px -5px rgb(0 0 0 / 14%) !important">
        ...
        <!-- Notification Start -->
        <v-menu v-model="menu" :close-on-content-click="false" :nudge-width="200" offset-x width="350">
                <template v-slot:activator="{ on, attrs }">
                        <v-btn color="indigo" dark v-bind="attrs" v-on="on" icon>
                                <v-badge v-if="notificationUnseen > 0" :content="notificationUnseen" class="badge-notification"></v-badge>
                        ...
                        <v-list three-line class="pa-0" style="overflow-y: scroll; height: 350px">
                                <template v-for="(notification, index) in notification">
                                        <div :key="index">
                                                ...
                                                <v-list-item :to="notification.link" :class="notification.seen ? '' : 'teal lighten-5'" @click="seenCall(notification.id)">
                                                        <v-list-item-avatar class="green" max-height="20px" max-width="20px" min-width="20px" style="margin-top: 20px">
                                                                <!-- <v-img :src="notification.avatar"></v-img> -->
                                                        </v-list-item-avatar>

                                                        <v-list-item-content>
                                                                <v-list-item-title v-html="notification.title" class="font-weight-bold"></v-list-item-title>
                                                                <v-list-item-subtitle>{{
                    notification.body
                  }}</v-list-item-subtitle>
                                                                <v-list-item-subtitle>
                                                                        <span class="font-weight-bold">{{ dateToYMD(notification.created_at) }} at
        ...
        <!-- User link Start -->
        <v-menu transition="scale-transition" origin="top end 0" v-model="user_menu" :close-on-content-click="false" offset-y :nudge-left="150">
                <template v-slot:activator="{ on, attrs }">
                        <v-btn variant="text" class="profileBtn custom-hover-primary" v-bind="attrs" v-on="on" icon>
                                ...
                                <v-avatar min-width="40" width="40" height="40">
                                        <img v-if="$store.state.user && $store.state.user.photo" :src="$store.state.user.photo.img_url" />
                                        <div v-else class="white--text d-flex justify-center align-center primary_bg" style="height: 100%; width: 100%">
                                                <v-avatar size="35">
